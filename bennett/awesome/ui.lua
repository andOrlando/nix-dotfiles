...
local function create_tasklist_item(layout)
	...
	local bg_timed = rubato.timed { duration = 0.2 }
	local drag_timed = rubato.timed { duration = 0.2 }
	local focus_timed = rubato.timed { duration = 0.2 }
...
local function create_toggle_button(icon, id)
	...
	--let me set state
	w.state = rubato.timed {duration=0.2}
...
local function create_info_widgets()
	...
	local w = wibox.widget {
		...
		bg = "#262930",
		shape = gears.shape.rounded_rect,
		widget = coolwidget.margin.background.stack,
	}
