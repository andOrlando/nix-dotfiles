...
local function create_brightness_widget()
	...
	awful.spawn.with_line_callback("brightnessctl g", {
		stdout = function(out) 
			current = out 
			if current and max then brightness_slider:hard_set(current / max) end
		end
	})
	...
	brightness_slider:connect_signal("slider::moved", function(_, pos)
		awful.spawn("brightnessctl s "..math.floor(5 + pos * 95).."%")
	end)
...
off_rgb.r, off_rgb.g, off_rgb.b = color.utils.hex_to_rgba(beautiful.bg_normal_1)
on_rgb.r, on_rgb.g, on_rgb.b = color.utils.hex_to_rgba(beautiful.light_blue)
...
return function(s)
	...
	blue_light_toggle = toggle_button(beautiful.lightbulb, awful.button({}, 1, function()
			...
			awful.spawn("xsct "..tostring(6500 - toggle.blue * 2000))
