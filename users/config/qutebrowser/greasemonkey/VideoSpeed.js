...
function handlePressedKey(event) {
  ...
  const key = event.key;
  if (key === ",") video.playbackRate -= 0.2;
  else if (key === ".") video.playbackRate += 0.2;
  else if (key === ";") video.playbackRate = 1;
  else if (key === "[") video.playbackRate = 1.8;
  else if (key === "]") video.playbackRate = 2.4;
  else if (key === "\\") video.playbackRate = prompt("Enter a float video speed") || video.playbackRate;
  else if (key === "|") alert(video.playbackRate);
